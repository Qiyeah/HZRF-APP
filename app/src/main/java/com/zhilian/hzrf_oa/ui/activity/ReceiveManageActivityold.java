 ...
 * 收文管理
 */
public class ReceiveManageActivityold extends Activity {
	private ListView listView;// 收文管理列表
	private IncomingManageAdapter adapter;// 收文管理列表的适配器
	private List<DocManage> adapterlist = new ArrayList<DocManage>();
	private List<T_Receive> list;

	private ImageView back,ivDeleteText;// 返回
	private EditText editText;

	private Handler myhandler = new Handler();
	private ArrayList<Map<String, Object>> mData = new ArrayList<Map<String, Object>>();
	private ArrayList<String> mListTitle = new ArrayList<String>();
	private ArrayList<String> mListDocno = new ArrayList<String>();
	private ArrayList<String> mListDate = new ArrayList<String>();
	private ArrayList<String> mListUnit = new ArrayList<String>();
	private ArrayList<Integer> mListDocid = new ArrayList<Integer>();
	private ArrayList<Integer> mListPid = new ArrayList<Integer>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.receive_list);
		listView = (ListView) findViewById(R.id.list_incoming_manage);
		addData();// 添加数据
		initView();// 初始化
	}
	...
	private void initView() {
		...
		listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				Intent intent = new Intent(ReceiveManageActivityold.this,ReceiveDetail.class);
	...
	private void addData() {
		...
		inQueryMsg.setQueryName("getReceiveList");
				...
				new Response.Listener<String>() {
					...
					public void onResponse(String response) {
						...
						if (response.toString().equals("[]")) {
							Toast.makeText(ReceiveManageActivityold.this, "没有待处理的收文！", Toast.LENGTH_SHORT).show();
						} else {
							...
							/*for (int i = 0; i < list.size(); i++) {
								...
								int docid = list.get(i).getDocid();
								int id = list.get(i).getPid();
